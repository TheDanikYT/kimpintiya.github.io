...
const WindowManager = {
    ...
    open(id, arg = null) {
        ...
        head.innerHTML = `<div class="window-title">${id === 'browser' ? 'MakuBrowser' : id.toUpperCase()}</div><div class="window-controls"><button class="win-btn btn-min"></button><button class="win-btn btn-max"></button><button class="win-btn btn-close"></button></div>`;
    ...
    close(id) { 
        if(this.active[id] && this.active[id].appId === 'browser') {
            Browser.cleanup(this.active[id].el);
        }
        this.active[id].el.remove(); 
    ...
    updateTb() {
        ...
        for(let id in this.active) {
            ...
            if(appId==='settings')i='ph-gear'; if(appId==='notepad')i='ph-notepad'; if(appId==='explorer')i='ph-folder'; if(appId==='calculator')i='ph-calculator'; if(appId==='paint')i='ph-palette'; if(appId==='cmd')i='ph-terminal'; if(appId==='browser')i='ph-browser';
...
const Browser = {
    ...
    init(win) {
        // Инициализация вкладок для нового окна
        const tabsContainer = win.querySelector('#browser-tabs');
        const contentContainer = win.querySelector('#browser-content');
        
        // Очищаем контейнеры от дефолтного содержимого
        tabsContainer.innerHTML = '';
        contentContainer.innerHTML = '';
        
        // Создаем первую вкладку
        this.createTab(win, 'https://www.google.com');
    },
    
    createTab(win, url = 'https://www.google.com') {
        ...
        tab.innerHTML = `
            <span>${this.getTabTitle(url)}</span>
            <button class="tab-close" onclick="Browser.closeTab(event, this)"><i class="ph ph-x"></i></button>
        `;
        tab.onclick = (e) => {
        ...
        iframe.id = `browser-frame-${tabId}`;
        iframe.className = 'browser-frame';
        iframe.src = url;
        iframe.sandbox = 'allow-same-origin allow-scripts allow-popups allow-forms allow-modals allow-popups-to-escape-sandbox';
        iframe.onload = () => this.updateUrlBar(win, iframe, tabId);
        iframe.onerror = () => this.showError(win, iframe);
        ...
        // Обновляем URL бар
        this.updateUrlBar(win, iframe, tabId);
    ...
    getTabTitle(url) {
        try {
            ...
            return hostname.replace('www.', '').split('.')[0] || 'Новая вкладка';
        } catch {
    ...
    switchTab(win, tabId) {
        // Деактивируем все вкладки и iframe
        win.querySelectorAll('.browser-tab').forEach(tab => {
            tab.classList.remove('active');
        });
        win.querySelectorAll('.browser-frame').forEach(frame => {
            frame.classList.remove('active');
        });
        
        // Активируем выбранную вкладку
        const selectedTab = win.querySelector(`.browser-tab[data-tab-id="${tabId}"]`);
        const selectedFrame = win.querySelector(`#browser-frame-${tabId}`);
        
        if(selectedTab) selectedTab.classList.add('active');
        if(selectedFrame) {
            selectedFrame.classList.add('active');
            this.updateUrlBar(win, selectedFrame, tabId);
        }
        
        // Обновляем кнопки навигации
        this.updateNavButtons(win, selectedFrame);
    },
    
    updateUrlBar(win, iframe, tabId) {
        const urlBar = win.querySelector('#browser-url');
        if(urlBar && iframe && iframe.contentWindow) {
            try {
                const currentUrl = iframe.contentWindow.location.href;
                urlBar.value = currentUrl;
                
                // Обновляем название вкладки
                const tab = win.querySelector(`.browser-tab[data-tab-id="${tabId}"]`);
                if(tab) {
                    const title = this.getTabTitle(currentUrl);
                    const span = tab.querySelector('span');
                    if(span) span.textContent = title;
                }
            } catch(e) {
                // CORS ограничения, игнорируем
                console.log('Cannot access iframe URL due to CORS');
            }
        }
    },
    
    updateNavButtons(win, iframe) {
        const backBtn = win.querySelector('#browser-back');
        const forwardBtn = win.querySelector('#browser-forward');
        
        if(iframe && iframe.contentWindow) {
            try {
                backBtn.disabled = !iframe.contentWindow.history?.canGoBack;
                forwardBtn.disabled = !iframe.contentWindow.history?.canGoForward;
            } catch(e) {
    ...
    goBack(btn) {
        ...
        const activeFrame = win.querySelector('.browser-frame.active');
        if(activeFrame && activeFrame.contentWindow) {
            try {
                activeFrame.contentWindow.history.back();
                setTimeout(() => this.updateUrlBar(win, activeFrame, activeFrame.id.split('-')[2]), 100);
            } catch(e) {
    ...
    goToUrl(btn) {
        ...
        if(activeFrame) {
            activeFrame.src = url;
            const statusBar = win.querySelector('#browser-status');
            if(statusBar) statusBar.textContent = 'Загрузка...';
            
            activeFrame.onload = () => {
                if(statusBar) statusBar.textContent = 'Готово';
                this.updateUrlBar(win, activeFrame, activeFrame.id.split('-')[2]);
            };
        }
    ...
    closeTab(event, btn) {
        ...
        const tab = btn.closest('.browser-tab');
        const win = tab.closest('.os-window');
        const tabId = tab.dataset.tabId;
        const tabs = win.querySelectorAll('.browser-tab');
        const frames = win.querySelectorAll('.browser-frame');
        
        if(tabs.length === 1) {
            // Закрываем окно браузера если это последняя вкладка
            const windowId = Object.keys(WindowManager.active).find(id => WindowManager.active[id].el === win);
            if(windowId) WindowManager.close(windowId);
            return;
        }
        
        // Удаляем вкладку и соответствующий iframe
        tab.remove();
        const frame = win.querySelector(`#browser-frame-${tabId}`);
        if(frame) frame.remove();
        
        // Активируем первую вкладку
        const firstTab = win.querySelector('.browser-tab');
        if(firstTab) {
            this.switchTab(win, firstTab.dataset.tabId);
        }
    },
    
    showError(win, iframe) {
        const statusBar = win.querySelector('#browser-status');
        if(statusBar) statusBar.textContent = 'Ошибка загрузки';
    },
